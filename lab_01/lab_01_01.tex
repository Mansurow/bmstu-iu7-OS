...
074D  B8 0040				mov	ax,40h
0750  8E D8				    mov	ds,ax
0752  33 C0				    xor	ax,ax			
...
0756  FF 06 006C		    inc	word ptr ds:[6Ch]   	
;; (0040:006C=4E47h), по этому адресу располагается счетчик реального времен
075A  75 04				    jnz	loc_3			        ; Jump if not zero
075C  FF 06 006E			inc	word ptr ds:[6Eh]	    ; (0040:006E=15h)
...
0760			      loc_3:
0760  83 3E 006E 18			cmp	word ptr ds:[6Eh],18h	; (0040:006E=15h)
0765  75 15				    jne	loc_4			        ; Jump if not equal
0767  81 3E 006C 00B0		cmp	word ptr ds:[6Ch],0B0h	; (0040:006C=4E47h)
076D  75 0D				    jne	loc_4			        ; Jump if not equal

;; Обнуление счетчика (старшего слова и младшего слова) таймера
076F  A3 006E				mov	word ptr ds:[6Eh],ax	; (0040:006E=15h)
0772  A3 006C				mov	word ptr ds:[6Ch],ax	; (0040:006C=4E47h)

;; Прошло более 24 часов, занесение значения 1 в 0040:0070
0775  C6 06 0070 01			mov	byte ptr ds:[70h],1	    ; (0040:0070=0)
;; Установка al = 8:
077A  0C 08				    or	al,8
...
077D  FE 0E 0040			dec	byte ptr ds:[40h]		; (0040:0040=96h)
0781  75 0B					jnz	loc_5					; Jump if not zero
;; Установка флагов, отвечающих за отключение моторчикка дисковода:
0783  80 26 003F F0			and	byte ptr ds:[3Fh],0F0h	; (0040:003F=0)
;; Отправка команды отключения моторчика дисковода:
0788  B0 0C					mov	al,0Ch
078A  BA 03F2				mov	dx,3F2h
078D  EE					out	dx,al					; port 3F2h, dsk0 contrl output
